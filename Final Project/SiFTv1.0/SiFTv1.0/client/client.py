...
if __name__ == '__main__':
    """
    A socket is an endpoint for sending or receiving data across a computer network. It allows communication between two machines over a network, typically using the Internet Protocol (IP). Sockets are used to establish a connection between a client and a server, enabling them to exchange data.
    """
    try:
    ...
    mtp = SiFT_MTP(sckt) # so create the mtp protocol
    loginp = SiFT_LOGIN(mtp) # then hadle the first subprotocol aka loginp by creatitn a new login instance

    print()
    username = input('   Username: ')
    password = getpass.getpass('   Password: ')
    # should the timestamp be calculated at this point? OR what does the timestamp represent
    # when the login request was generated.
    print()

    try:
        # so I guess here
        loginp.handle_login_client(username, password) # handle this request which is a login req from the client
